...
while {true} do 
{
	...
	if (!(vehicle player == player) && driver vehicle player == player) then {_LandVehicle = vehicle player;BTC_towHudId = _LandVehicle addAction [("<t color=""#ED2744"">" + ("") + "</t>"),"=BTC=_LogisticTow\=BTC=_tow\=BTC=_Hud.sqf", "", 0, false, false];};
