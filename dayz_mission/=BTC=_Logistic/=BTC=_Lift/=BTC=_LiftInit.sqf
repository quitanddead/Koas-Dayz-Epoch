...
while {true} do 
{
	...
	if (!(vehicle player == player) && driver vehicle player == player) then {_chopper = vehicle player;BTC_liftHudId = _chopper addAction [("<t color=""#ED2744"">" + ("") + "</t>"),"=BTC=_Logistic\=BTC=_Lift\=BTC=_Hud.sqf", "", 0, false, false];};
	...
	while {!(vehicle player == player) && driver vehicle player == player} do
	{
		...
		if (_cargo isKindOf "StaticWeapon" && getdammage _cargo != 1) then {_can_lift = false;};
