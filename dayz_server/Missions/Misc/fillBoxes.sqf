_crate = _this select 0;

clearWeaponCargoGlobal _crate;
clearMagazineCargoGlobal _crate;



_crate addWeaponCargoGlobal ["ItemToolbox", 1];

_crate addMagazineCargoGlobal ["Skin_Soldier1_DZ", 2];

_crate addMagazineCargoGlobal ["Skin_Camo1_DZ", 2];


_crate addWeaponCargoGlobal ["glock17_EP1", 1];

_crate addWeaponCargoGlobal ["UZI_EP1", 1];



_crate addMagazineCargoGlobal ["17Rnd_9x19_glock17", 3];

_crate addMagazineCargoGlobal ["30Rnd_9x19_UZI", 3];



//_crate addWeaponCargoGlobal ["G36A_camo", 1];

_crate addWeaponCargoGlobal ["M4A1_AIM_SD_camo", 1];

//_crate addWeaponCargoGlobal ["G36C_camo", 2];

_crate addWeaponCargoGlobal ["M14_EP1", 2];

//_crate addWeaponCargoGlobal ["M16A4_ACG_GL", 1];

_crate addWeaponCargoGlobal ["M16A4_GL", 2];

//_crate addWeaponCargoGlobal ["M4A3_CCO_EP1", 1];

_crate addWeaponCargoGlobal ["M4A1_AIM_SD_camo", 2];

//_crate addWeaponCargoGlobal ["M4SPR", 1];

_crate addWeaponCargoGlobal ["RPK_74", 1];

_crate addWeaponCargoGlobal ["Sa58V_RCO_EP1", 2];

//_crate addWeaponCargoGlobal ["M4A1_HWS_GL_SD_Camo", 1];

_crate addBackpackCargoGlobal ['DZ_Backpack_EP1', 1];


_crate addMagazineCargoGlobal ["30Rnd_545x39_AKSD", 3];

_crate addMagazineCargoGlobal ["30Rnd_762x39_AK47", 3];

_crate addMagazineCargoGlobal ["30Rnd_762x39_SA58", 3];

_crate addMagazineCargoGlobal ["30Rnd_556x45_G36", 3];

_crate addMagazineCargoGlobal ["30Rnd_556x45_StanagSD", 3];

_crate addMagazineCargoGlobal ["30Rnd_556x45_Stanag", 3];

_crate addMagazineCargoGlobal ["20Rnd_556x45_Stanag", 3];

_crate addMagazineCargoGlobal ["75Rnd_545x39_RPK", 3];


_crate addMagazineCargoGlobal ["1Rnd_SmokeRed_M203", 3];

_crate addMagazineCargoGlobal ["1Rnd_SmokeGreen_M203", 3];

_crate addMagazineCargoGlobal ["1Rnd_Smoke_M203", 3];


_crate addMagazineCargoGlobal ["SmokeShellRed", 3];

_crate addMagazineCargoGlobal ["SmokeShellGreen", 3];

_crate addMagazineCargoGlobal ["SmokeShellPurple", 3];

_crate addMagazineCargoGlobal ["SmokeShellBlue", 3];

_crate addMagazineCargoGlobal ["SmokeShellYellow", 3];
